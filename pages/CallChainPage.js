import { LitElement, html, svg } from 'https://cdn.skypack.dev/lit-element';
...
export default class CallChainPage extends baseClass {
	...
	render() {
		...
		const totalWidth = columnsArray.reduce((acc, [x, map], columnIndex) => {
			console.assert(map, 'No map found');
			console.assert(map.size, 'No size found');
		
			const colWidth = 200; // Width of each column
			const colGap = 100;   // Gap between columns
		
			// Calculate the total width based on the number of columns and gaps
			return acc + colWidth + (columnIndex > 0 ? colGap : 0);
		}, 0);
		...
		columnsArray.forEach(([x, map]) => {
			Array.from(map).forEach(([y]) => {
		...
		// Calculate the total height based on the maximum number of rows and gaps
		const totalHeight = (maxRow + 1) * (rowHeight + rowGap);
